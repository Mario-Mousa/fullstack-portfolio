...
import {
  applyPortfolioLanguageToDocument,
  nextPortfolioLanguage,
  toPortfolioLanguage,
  type PortfolioLanguage,
} from "@/lib/portfolioLanguage";
...
const tabs: {
  id: TabId;
  hash: string;
  label: "about" | "skills" | "projects" | "certificates" | "contact";
}[] = [
  { id: "about", hash: "about", label: "about" },
  { id: "skills", hash: "skills", label: "skills" },
  { id: "projects", hash: "work", label: "projects" },
  { id: "certificates", hash: "credentials", label: "certificates" },
...
export default function Home() {
  ...
  const { data, isLoading, isError, refetch } =
    trpc.portfolio.publicData.useQuery();
  const [language, setLanguage] = useState<PortfolioLanguage>(() =>
    toPortfolioLanguage(i18n.resolvedLanguage ?? i18n.language)
  );
  ...
  useEffect(() => {
    const syncLanguageState = (nextLanguage: string) =>
      setLanguage(toPortfolioLanguage(nextLanguage));
  ...
  const changeLanguage = () => {
    ...
    void i18n
      ...
      .catch(() =>
        toast.error(
          "Could not switch the portfolio language. Please try again."
        )
      );
  };
  ...
  const availability = isArabic
    ? profile?.availabilityAr
  ...
  if (isError)
    return (
      <main className="load-error">
        <div className="load-error-card glass-panel">
          <Code2 size={28} />
          <h1>{t("portfolio.loadError")}</h1>
      ...
      <div className="ambient-field" aria-hidden="true">
        ...
        <div className="ambient-network">
          <svg viewBox="0 0 900 520" preserveAspectRatio="none">
            <path d="M32 90 170 165 302 88 465 196 604 122 782 211 882 98M78 420 216 338 386 432 528 318 690 410 860 336" />
          </svg>
          <i />
          <i />
          <i />
        ...
        <aside className="identity-rail" aria-label="Profile summary">
          <div className="identity-card glass-panel">
            <ProfileLightbox
              ...
              initials={initials}
            />
            <p className="rail-system">SYSTEM // 01</p>
            <h2>{name || "Portfolio"}</h2>
            <p className="rail-headline">{headline || ""}</p>
            {availability ? (
          ...
          <nav className="identity-navigation" aria-label="Portfolio sections">
            {tabs.map((tab, index) => (
              <button
                type="button"
                key={tab.id}
        ...
        <div className="content-canvas">
          <header className={`topbar ${scrolled ? "topbar-scrolled" : ""}`}>
            ...
            <div className="nav-controls">
              ...
              <button
                className="icon-button compact mobile-menu-button"
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === "about" ? (
                <>
                  <section id="about" className="hero section-shell rail-hero">
              ...
              {activeTab === "skills" ? (
                <section
                  id="skills"
                  className="section-shell content-section skills-section tab-section"
                >
                  <Reveal>
                    <SectionHeading
                      index="02"
                  ...
                  <div className="skills-layout">
                    <Reveal delay={0.08} className="skill-feature">
                      <div className="skill-feature-icon">
                        <Globe2 size={24} />
                      </div>
                      <div>
                        <p className="eyebrow">
                        ...
                        <h3>
                          {isArabic
                            ? "من الـ API إلى تجربة المستخدم."
                            : "From the API to the user experience."}
                        </h3>
                      </div>
                    </Reveal>
                    <div className="skills-cloud">
                      {(profile?.skills ?? []).map((skill, index) => (
                        <Reveal key={skill} delay={0.04 * index}>
                          <span
                            className={`skill-pill skill-pill-${index % 4}`}
                          >
                            {skill}
                          </span>
                        </Reveal>
              ...
              {activeTab === "projects" ? (
                <section
                  id="work"
                  className="section-shell content-section projects-section tab-section"
                >
                  <Reveal>
                    <SectionHeading
                      index="03"
                    ...
                    <div className="projects-grid">
                      {data?.projects.map((project, index) => (
                        <Reveal key={project.id} delay={0.06 * index}>
                          <article
                            className={`project-card glass-panel ${index % 3 === 0 ? "project-card-featured" : ""}`}
                          >
              ...
              {activeTab === "certificates" ? (
                <section
                  id="credentials"
                  className="section-shell content-section certificates-section tab-section"
                >
                  <Reveal>
                    <SectionHeading
                      index="04"
                      ...
                      copy={t("certificates.copy")}
                    />
                  </Reveal>
                  {(data?.certificates?.length ?? 0) > 0 ? (
                    <div className="certificates-list">
                      {data?.certificates.map((certificate, index) => (
                        <Reveal key={certificate.id} delay={0.05 * index}>
              ...
              {activeTab === "contact" ? (
                <section
                  id="contact"
                  className="section-shell content-section contact-section tab-section"
                >
                  <Reveal>
          ...
          <footer className="footer section-shell">
            ...
            <div className="footer-links">
              ...
              <a href="/admin/login" className="footer-dashboard">
                {t("footer.dashboard")}
