...
type TabId = "about" | "skills" | "projects" | "certificates" | "contact";

const tabs: { id: TabId; hash: string; label: "about" | "skills" | "projects" | "certificates" | "contact" }[] = [
  { id: "about", hash: "about", label: "about" }, { id: "skills", hash: "skills", label: "skills" },
  { id: "projects", hash: "work", label: "projects" }, { id: "certificates", hash: "credentials", label: "certificates" }, { id: "contact", hash: "contact", label: "contact" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    const syncTabWithUrl = () => {
      ...
      setActiveTab(tabs.find(tab => tab.hash === hash)?.id ?? "about");
  ...
  const selectTab = (tabId: TabId) => {
    ...
    if (!tab) return;
    if (window.location.hash !== `#${tab.hash}`) window.history.pushState(null, "", `#${tab.hash}`);
    setActiveTab(tabId);
    setMenuOpen(false);
    window.scrollTo(0, 0);
  };
  ...
  return (
    <main className="portfolio-shell signal-garden">
      ...
      <aside className="identity-rail" aria-label="Profile summary">
        ...
        <nav className="identity-navigation" aria-label="Portfolio sections">{tabs.map((tab, index) => <button type="button" key={tab.id} onClick={() => selectTab(tab.id)} data-active={activeTab === tab.id}><span>0{index + 1}</span>{t(`nav.${tab.label}`)}</button>)}</nav>
      </aside>
      <div className="content-canvas">
      <header className={`topbar ${scrolled ? "topbar-scrolled" : ""}`}>
        <button type="button" onClick={() => selectTab("about")} className="brand" aria-label="Portfolio home"><span className="brand-mark brand-monogram" aria-hidden="true"><i /><i /><i /></span><span>{name || "Portfolio"}</span></button>
        <nav className="desktop-nav" aria-label="Main navigation">
          {tabs.map(tab => <button type="button" key={tab.id} onClick={() => selectTab(tab.id)} data-active={activeTab === tab.id}>{t(`nav.${tab.label}`)}</button>)}
        </nav>
        ...
        {menuOpen ? <nav className="mobile-nav glass-panel" aria-label="Mobile navigation">{tabs.map(tab => <button type="button" key={tab.id} onClick={() => selectTab(tab.id)} data-active={activeTab === tab.id}>{t(`nav.${tab.label}`)}</button>)}</nav> : null}
      </header>

      <AnimatePresence mode="wait">
      <motion.div key={activeTab} className={`tab-panel tab-panel-${activeTab}`} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.24, ease: [0.23, 1, 0.32, 1] }}>
      {activeTab === "about" ? <><section id="about" className="hero section-shell rail-hero">
      ...
      <section className="section-shell content-section about-section">
      ...
      {activeTab === "skills" ? <section id="skills" className="section-shell content-section skills-section tab-section">
      ...
      {activeTab === "projects" ? <section id="work" className="section-shell content-section projects-section tab-section">
      ...
      {activeTab === "certificates" ? <section id="credentials" className="section-shell content-section certificates-section tab-section">
      ...
      {activeTab === "contact" ? <section id="contact" className="section-shell content-section contact-section tab-section">
